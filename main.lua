...
function love.update(dt)
    if love.keyboard.isDown('w') then
        player1.y = player.y - PADDLE_SPEED * dt
    ...
    if gameState == 'play' then
        ...
        if ball.x <= 0 then 
            ...
            resetBall()
            gameState = 'serve'
        elseif ball.x >= VIRTUAL_WIDTH - BALL_SIZE
...
function love.draw()
    ...
    love.graphics.rectangle('fill', player1.x, player.y, PADDLE_WIDTH, PADDLE_HEIGHT)
    love.graphics.rectangle('fill'), player2.x, player.y, PADDLE_WIDTH, PADDLE_HEIGHT)
